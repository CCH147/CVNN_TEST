...
for snr in SNR_range:
    ...
    for i in range(samples_per_snr):
        ...
        # 使用RMS归一化
        rx_power = np.sqrt(np.mean(np.abs(rx) ** 2))
        rx_norm = rx / (rx_power + 1e-12)
        
        # 保存干净信号用于分析
        s_norm = s / (rx_power + 1e-12)

        dataset.append({
            'rx': rx_norm.astype(np.complex64),      # 带噪声
            's': s_norm.astype(np.complex64),        # 干净信号
            'a': a.astype(np.complex64),             # 目标
            'snr': np.float32(snr)                   # SNR标签
        })
